...
let () =
  main ~name:"discover" (fun t ->
      ...
      let result_to_string = function
        ...
        | Result.Error e -> "Result.error (Rresult.R.msg \"" ^ (String.escaped e) ^ "\")"
      in
      let v1result_to_string r = match r, platformname with
        | _, (Result.Ok "linux_x86") -> "Result.error (Rresult.R.msg \"linux_x86 platform is only available in Target_context.V2 or later\")"
      ...
      write_lines "target_context.ml"
        [
          ...
          {|  type platformtype =
              ...
              | Windows_arm32
          |};
          {|  let get_os : (ostype, Rresult.R.msg) Result.t Lazy.t = |} ^ (v1result_to_string ostypename |> to_lazy);
          {|  let get_platform : (platformtype, Rresult.R.msg) Result.t Lazy.t = |} ^ (v1result_to_string platformtypename |> to_lazy);
          {|  let get_platform_name : (string, Rresult.R.msg) Result.t Lazy.t = |} ^ (Result.map quote_string platformname |> v1result_to_string |> to_lazy);
          {|end (* module V1 *) |};
          {||};
          {|module V2 = struct|};
          {|  type ostype = Android | IOS | Linux | OSX | Windows|};
          {|  type platformtype =
              ...
              | Windows_arm32
          |};
          {|  let get_os : (ostype, Rresult.R.msg) Result.t Lazy.t = |} ^ (result_to_string ostypename |> to_lazy);
          {|  let get_platform : (platformtype, Rresult.R.msg) Result.t Lazy.t = |} ^ (result_to_string platformtypename |> to_lazy);
          {|  let get_platform_name : (string, Rresult.R.msg) Result.t Lazy.t = |} ^ (Result.map quote_string platformname |> result_to_string |> to_lazy);
          {|end (* module V2 *) |};
        ])

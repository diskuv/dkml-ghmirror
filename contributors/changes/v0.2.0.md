...
* [Windows only] To work around a LINK.EXE bug during 32-bit builds, downgraded
  Visual Studio Build Tools 16.11 to 16.6 and downgraded the compiler component
  `Microsoft.VisualStudio.Component.VC.Tools.x86.x64` ("MSVC v142 - VS 2019 C++ x64/x86 build tools (Latest)") to
  `Microsoft.VisualStudio.Component.VC.14.26.x86.x64` ("MSVC v142 - VS 2019 C++ x64/x86 build tools (v14.26)").

### Upgrading from v0.1.0 or v0.1.1 to v0.2.0

You will need to:
* upgrade your *Diskuv OCaml* system if you are on Windows
* upgrade your Local Projects (ex. [diskuv-ocaml-starter](https://gitlab.com/diskuv/diskuv-ocaml-starter))
  which can be done at your leisure **before** the next system upgrade
* downgrade Visual Studio Build Tools 2019 and Visual Studio Installer if you are on Windows. You can
  leave untouched any other Visual Studio installation.
...
THIRD, you will also need to downgrade Visual Studio Build Tools 2019.
1. Press the Windows Key to get the "Type here to search" box. Type and run "Visual Studio Installer". Alternatively, in PowerShell run: `& "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\setup.exe"`
2. Select the "More ... Uninstall" action beside "Visual Studio Build Tools 2019".
   ![Remove Visual Studio Build Tools by clicking More then Uninstall](v0.2.0.VisualStudioInstaller-RemoveVisualStudioBuildTools2019.png)
3. Run the following in PowerShell to reinstall correct versions of "Visual Studio Installer" and "Visual Studio Build Tools 2019":
   ```powershell
   ~\DiskuvOCamlProjects\diskuv-ocaml\installtime\windows\setup-machine.bat
   ```
...
Known issues:
* During `opam install` the local fdopen-mingw repository ~20MB tarball may be repackaged. If you have several packages and their
  dependencies to install, this disk I/O may significantly slow down your installation. Some of the performance degradation will
  be improved with the future trimming of that repository; see the *Deprecated* section below for more details.

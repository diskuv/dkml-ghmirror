...
* The first `opam dkml init` takes ~20 minutes to build all of its dependencies.
  A future release may drop the Opam plugin (the `opam dkml ...`) entirely
  and just distribute a precompiled binary (ex. `dkml init`).

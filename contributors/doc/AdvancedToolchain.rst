...
.. code-block:: shell-session
    ...
    [core_kernel.v0.14.2]$ opam update diskuv-1.0.1
...
---------------------------------
    ...
    ``opam update diskuv-1.0.1 && opam upgrade`` before doing any of these
...
If after ``opam update diskuv-1.0.1`` you get:
...
then rerun the command as ``opam update diskuv-1.0.1 -vv``. That will give you
something like:
...
.. code:: bash
    ...
    opam repository remove diskuv-1.0.1 --all

    # On Windows do: ./makeit init-dev
    make init-dev

    opam repository priority diskuv-1.0.1 1 --all
    opam update diskuv-1.0.1

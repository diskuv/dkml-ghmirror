...
1. diskuv-0.4.1
2. fdopen-mingw-0.4.1 *only for Windows*
3. default

The ``diskuv-0.4.1`` repository has all the patches required for third-party OCaml packages
...
The ``fdopen-mingw-0.4.1`` repository has all the MinGW patches for _many_ third-party OCaml packages
to work with MinGW (an alternative compiler popular on Linux). Unlike ``diskuv-0.4.1`` the packages

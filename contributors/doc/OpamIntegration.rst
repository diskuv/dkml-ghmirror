...
1. diskuv-0.2.3
2. fdopen-mingw-0.2.3 *only for Windows*
3. default

The ``diskuv-0.2.3`` repository has all the patches required for third-party OCaml packages
...
The ``fdopen-mingw-0.2.3`` repository has all the MinGW patches for _many_ third-party OCaml packages
to work with MinGW (an alternative compiler popular on Linux). Unlike ``diskuv-0.2.3`` the packages
...
.. code-block:: bash
    ...
    autodetect_compiler /tmp/launcher.sh "$LOCALAPPDATA/opam/plugins/diskuvocaml/vcpkg/0.2.3/installed/x86-windows" # if 64-bit
    autodetect_compiler /tmp/launcher.sh "$LOCALAPPDATA/opam/plugins/diskuvocaml/vcpkg/0.2.3/installed/x64-windows" # if 32-bit

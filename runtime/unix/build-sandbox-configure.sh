...
TARGET_OPAMSWITCH=$RESOLVED_BUILDDIR/$DKMLPLATFORM/$BUILDTYPE

# -----------------------
# BEGIN opam switch create

DKML_FEATUREFLAG_CMAKE_PLATFORM=ON "$DKMLDIR"/vendor/drd/src/unix/create-opam-switch.sh -y -p "$DKMLPLATFORM" -d "$STATEDIR" -u "$USERMODE" -t "$TARGET_OPAMSWITCH" -b "$BUILDTYPE" -o "$OPAMHOME" -v "$OCAMLHOME"
...
if cmake_flag_on "$IS_DEV_MODE"; then
    ...
    DKML_FEATUREFLAG_CMAKE_PLATFORM=ON "$DKMLDIR"/vendor/drd/src/unix/private/platform-opam-exec.sh -p "$DKMLPLATFORM" -d "$STATEDIR" -u "$USERMODE" -t "$TARGET_OPAMSWITCH" -b "$BUILDTYPE" -o "$OPAMHOME" -v "$OCAMLHOME" list --short > "$WORK"/packages
    ...
    then
        ...
        {
            printf '%s\n' "DKML_FEATUREFLAG_CMAKE_PLATFORM=ON '$DKMLDIR'/vendor/drd/src/unix/private/platform-opam-exec.sh -p '$DKMLPLATFORM' -d '$STATEDIR' -u '$USERMODE' -t '$TARGET_OPAMSWITCH' -b '$BUILDTYPE' -o '$OPAMHOME' -v '$OCAMLHOME' install --jobs=$NUMCPUS --yes \\"
...
{
    # [configure.sh JOBS]
    printf '%s\n' "exec env DKML_FEATUREFLAG_CMAKE_PLATFORM=ON '$DKMLDIR'/vendor/drd/src/unix/private/platform-opam-exec.sh -p '$DKMLPLATFORM' -d '$STATEDIR' -u '$USERMODE' -t '$TARGET_OPAMSWITCH' -b '$BUILDTYPE' -o '$OPAMHOME' -v '$OCAMLHOME' install --jobs=\$1 --yes \\"
    if [ "${DKML_BUILD_TRACE:-OFF}" = ON ]; then printf '%s\n' "  --debug-level 2 \\"; fi
    printf '%s\n' "  --deps-only --with-test \\"
